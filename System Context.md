...
## 6. Database Schema Design (PostgreSQL/JSONB)

Designed to prevent bottlenecks and support real-time analytics by separating Master Data from Transactions.
...
erDiagram
    ...
    entities ||--o{ sentiment_results : "is_analyzed_in"
    entities ||--o{ analysis_details : "acts_as_speaker"
    articles ||--o{ sentiment_results : "contains"
    sentiment_results ||--o{ analysis_details : "broken_down_into"
...
* **Table: `entities`**: Stores the unique persona (`canonical_name`, `category`, `wikidata_id`, `lang`).
* **Table: `aliases`**: Stores name variations linked to `entities` (`alias_text`, `source_type`).

#### Category 2: Transactional Data (Analysis)

* **Table: `articles`**: Stores news metadata (`headline`, `source_url`, `publisher`, `lang`, `published_at`).
* **Table: `sentiment_results`**: Stores the final aggregated score per entity per news piece (`final_score`, `sentiment_label`).
* **Table: `analysis_details`**: Stores granular sentence-level data (`sentence_text`, `is_headline`, `speaker_id`, `raw_sentiment`, `reasoning`).

#### Relationship Explanation (Logic Breakdown)

* **One-to-Many (`entities` -> `aliases`):** One person (Entity) can have multiple names or aliases. This is very important for Entity Linking to ensure the map system can always find its way back to "Anutin".
* **One-to-Many (`articles` & `entities` -> `sentiment_results`):** One news article can analyze multiple people, and one person can appear in multiple news articles (via the central table `sentiment_results`).
* **One-to-Many (`sentiment_results` -> `analysis_details`):** For transparency. (Transparency) We won't just store the total score, but also the "inside details" of how SLM analyzes each sentence in the news article.
* **Optional Relationship (`entities` -> `analysis_details`):** `speaker_id` is a Foreign Key that links back to `entities` to identify who said that sentence (used to solve Quotation Contamination problems). If it's a journalist's narration, this value will be `NULL`.

#### Senior's Technical Advice (Database Level)

To make this project truly scalable... Here are some points to watch out for at the database level:

* **Composite Index:** I recommend creating an index on `(entity_id, published_at)` in the `sentiment_results` table. Most dashboards query for "this person's sentiment during X time periods." Creating this index will make your graphs load 10 times faster.
* **JSONB for Reasoning:** In the `analysis_details` table, I use `JSONB` for `reasoning` so you can store various metadata from the SLM (e.g., token usage or attention score) without frequently changing the schema.
* **Data Integrity:** Remember to include `ON DELETE CASCADE` in the `aliases` section, but for `sentiment_results`, use `RESTRICT` to prevent the deletion of the primary entity containing important sentiment history.